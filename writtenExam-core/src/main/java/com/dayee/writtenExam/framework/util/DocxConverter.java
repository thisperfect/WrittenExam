...
public class DocxConverter {
    ...
    public static void convert(String path, String output) throws Exception {
        ...
        converter.htmlDocumentFacade =  new HtmlDocumentFacade(DocumentBuilderFactory.newInstance().newDocumentBuilder().newDocument());
        // System.err.println(ctp.getDomNode());

        // TODO get page width && å†…è¾¹è·???? åœ¨å“ªå¾—åˆ°å€¼??!!!
        // POIXMLProperties prop = converter.docx.getProperties();
        // POIXMLProperties.CoreProperties core = prop.getCoreProperties();
        // POIXMLProperties.ExtendedProperties ex =
        // prop.getExtendedProperties();
    ...
    private void processParagraphMulti(XWPFParagraph paragraph, Element page, boolean isTableParagraph) throws IOException {
        ...
        if (paragraph.getAlignment() != ParagraphAlignment.LEFT) {
            ...
            case RIGHT:
                ...
                break;
            // case DISTRIBUTE:addStyle(ctner, ""); break; //ä¸¤è¾¹åˆ†å¸ƒ
            default:
                break;
            }
        }
    ...
    private void processImage(Element wrap, List<XWPFPicture> pics) throws IOException {
        ...
        for (XWPFPicture pic : pics) {
            ...
            {// add picture to html page
                // TODO get img relative path for showing html page when on
                // server && get the picture style, scaling in the docx file
                // (with description style?)
                Element img = htmlDocumentFacade.createImage(imgFullPath);
                if (!StringUtils.isEmpty(pic.getDescription())) {
    ...
    public static String convertDocxToHTML(String docxFilePath) throws FileNotFoundException, IOException {
        ...
        try {
            ...
            XHTMLOptions options = XHTMLOptions.create().indent( 4 );// .indent( 4 );
            // // Extract image
            ...
            XHTMLConverter.getInstance().convert(document, System.out, options);
...
//            out.writeTo(fos);
    ...
    public static void main(String[] args) throws Exception {
        convert("/Users/tang/Desktop/51job_田予诺_社交媒体运营专员（实习生）(346709883).docx", "/Users/tang/Desktop/test123.html");
    }

<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="GetBorderPr">
        ...
        <xsl:choose>
            <xsl:when test="@WX:bdrwidth">
                <xsl:value-of select="@WX:bdrwidth"/><xsl:value-of select="$sep2"/>
            </xsl:when>
        ...
        <xsl:for-each select="w:listPr">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:for-each select="($nsStyles[@w:styleId=$pstyleId])[1]/w:pPr[1]/w:listPr[1]">
                        <xsl:call-template name="PrsGetListPrCore">
                            <xsl:with-param name="type" select="$type"/>
                            <xsl:with-param name="prsR" select="$prsR"/>
                        </xsl:call-template>
    ...
    <xsl:template name="PrsGetListPrCore">
        ...
        <xsl:for-each select="$ndLists">
            ...
            <xsl:choose>
                <xsl:when test="w:listDef[@w:listDefId=$list/w:ilst/@w:val][1]/w:lvl[@w:ilvl=$ilvl][1]">
                ...
                <xsl:when test="w:listDef[@w:listDefId=$list/w:ilst/@w:val][1]/w:listStyleLink">
                    <xsl:variable name="linkedStyleId" select="w:listDef[@w:listDefId=$list/w:ilst/@w:val][1]/w:listStyleLink/@w:val" />
                    ...
                    <xsl:for-each select="w:listDef[@w:listDefId=$linkedList/w:ilst/@w:val][1]/w:lvl[@w:ilvl=$ilvl][1]">
                        <xsl:call-template name="PrsGetListPrFromListDef">
                            <xsl:with-param name="type" select="$type"/>
                            <xsl:with-param name="prsR" select="$prsR"/>
                        </xsl:call-template>
        ...
        <xsl:choose>
            ...
            <xsl:when test="w:listPr">
                ...
                <xsl:if test="w:listPr[1]/WX:t/@WX:wTabAfter">
                    ...
                    <span>
                        <xsl:attribute name="style">
                            <xsl:text>padding-left:</xsl:text><xsl:value-of select="(w:listPr[1]/WX:t/@WX:wTabAfter div 20)" /><xsl:text>pt;</xsl:text>
                        </xsl:attribute>
                    </span>
                </xsl:if>
            </xsl:when>
    ...
    <xsl:template name="DisplayR">
        ...
        <xsl:variable name="styleMod">
            ...
            <xsl:if test="$listDef/w:lvl[@w:ilvl=$ilvl]/w:rPr/w:rFonts/@w:ascii" >
    ...
    <xsl:template match="w:p">
        ...
        <xsl:if test="not(w:pPr/w:pStyle/@w:val='z-TopofForm') and not(w:pPr/w:pStyle/@w:val='z-BottomofForm')">
            <p>
                ...
                <span>
                    <xsl:attribute name="class"><xsl:value-of select="$pStyleId"/><xsl:value-of select="$charStyleSuffix"/></xsl:attribute>
                    <xsl:call-template name="DisplayPContent"><xsl:with-param name="b.bidi" select="$b.bidi"/><xsl:with-param name="prsR" select="$prsR.updated"/></xsl:call-template>
                </span>
            </p>
        </xsl:if>
    </xsl:template>
            ...
            <xsl:when test="w:frameset">
                <html>
                    <head>
                        ...
                        <title><xsl:for-each select="$ndOfficeDocPr/o:Title[1]"><xsl:value-of select="." /></xsl:for-each></title>
                    ...
                    <body lang="EN-US" style='tab-interval:.5in'>
                        <div class="Section1">
                            <p class="MsoNormal">This document cannot be displayed in the XML viewer. Please open the document in Microsoft Office Word 2003 to see the contents.</p>
                        </div>
                    </body>
                </html>
            </xsl:when>
